                ...
                Forms\Components\Section::make('Review Information')
                    ->schema([
                        Forms\Components\Select::make('job_id')
                            ->label('Job')
                            ->options(function () {
                                return \App\Models\Job::with(['homeowner', 'tradie'])->get()->mapWithKeys(function ($job) {
                                    $homeownerName = $job->homeowner ? trim($job->homeowner->first_name . ' ' . $job->homeowner->last_name) : 'Unknown';
                                    $tradieName = $job->tradie ? trim($job->tradie->first_name . ' ' . $job->tradie->last_name) : 'Unknown';
                                    return [$job->id => "Job #{$job->id} - {$homeownerName} → {$tradieName}"];
                                });
                            })
                            ->required()
                            ->searchable()
...
Tables\Columns\TextColumn::make('customer')
                    ...
                    ->sortable(query: function ($query, $direction) {
                        return $query->orderBy('homeowners.first_name', $direction);
                    })
                ...
                Tables\Columns\TextColumn::make('provider')
                    ...
                    ->sortable(query: function ($query, $direction) {
                        return $query->orderBy('tradies.first_name', $direction);
                    })
