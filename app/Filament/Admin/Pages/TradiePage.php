                        ...
                        Action::make('suspend')
                            ...
                            ->form([
                                \Filament\Forms\Components\Textarea::make('suspension_reason')
                                    ...
                                    ->placeholder('Describe why this tradie is being suspended...')
                                    ->required()
                                    ->rows(3),

                                \Filament\Forms\Components\Select::make('suspension_duration')
                                    ->label('Duration')
                                    ->options([
                                        '3 days' => '3 Days',
                                        '1 week' => '1 Week',
                                ...
                                \Filament\Forms\Components\DatePicker::make('custom_end_date')
                                    ->label('Custom End Date')
                                    ->visible(fn(callable $get) => $get('suspension_duration') === 'custom')
                                    ->minDate(now())
                                    ->required(fn(callable $get) => $get('suspension_duration') === 'custom'),
                            ])
                            ...
                            ->action(function (array $data, Tradie $record) {
                                ...
                                $endDate = match ($data['suspension_duration']) {
                                    ...
                                    'custom' => $data['custom_end_date'] ?? now()->addWeek(),
                                    default => now()->addWeek(),
                                };
                                ...
                                \Filament\Notifications\Notification::make()
                                    ->title("{$record->first_name} {$record->last_name} suspended")
                                    ->body("Reason: {$data['suspension_reason']}\nUntil: {$endDate->format('M d, Y')}")
                                    ->danger()
                                    ->send();

                                $this->dispatch('close-modal', id: 'viewProfile');
                                $this->dispatch('tradieUpdated');
                            }),
