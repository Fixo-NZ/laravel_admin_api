...
class ReviewController extends Controller
{
    ...
    {
        $review = Review::with(['homeowner', 'tradie'])
            ->where('job_id', $jobId)
            ->approved()
            ->first();
    ...
    public function reportReview(Request $request, $reviewId)
    {
        ...
        $existingReport = ReviewReport::where('review_id', $reviewId)
            ->where('reporter_type', get_class(auth()->user()))
        ...
        $report = ReviewReport::create([
            'review_id' => $reviewId,
            'reporter_type' => get_class(auth()->user()),
            'reporter_id' => auth()->id(),
    ...
    /**
     * Get statistics for a tradie
     */
    public function getTradieStats($tradieId)
    {
        $totalReviews = Review::forTradie($tradieId)->approved()->count();
        $averageRating = Review::getTradieAverageRating($tradieId);
        $ratingBreakdown = Review::getTradieRatingBreakdown($tradieId);
        ...
        $detailedRatings = Review::forTradie($tradieId)
            ->approved()
            ->select([
