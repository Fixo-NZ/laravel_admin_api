        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="bg-white border rounded-lg p-6 shadow-sm">
                <div class="flex flex-col items-center text-center">
                    <div class="h-24 w-24 rounded-full bg-indigo-100 flex items-center justify-center text-indigo-600 text-3xl font-bold">{{ strtoupper(substr($homeowner->first_name ?? '',0,1)) }}{{ strtoupper(substr($homeowner->last_name ?? '',0,1)) }}</div>
                ...
                <div class="mt-6 space-y-3">
                    ...
                    <div class="flex items-center justify-between text-sm text-gray-600">
                        ...
                        <span class="px-2 py-1 rounded-full text-xs font-semibold {{ $homeowner->status === 'active' ? 'bg-green-100 text-green-800' : ($homeowner->status === 'inactive' ? 'bg-red-100 text-red-800' : 'bg-yellow-100 text-yellow-800') }}">{{ ucfirst($homeowner->status ?? 'unknown') }}</span>
                    </div>
                </div>
            </div>

            <!-- Right: Details & Jobs (span two columns on lg) -->
